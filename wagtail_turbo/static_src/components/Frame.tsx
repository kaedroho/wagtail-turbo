...
const turboViews: Map<string, (data: any, csrfToken: string) => ReactNode> = new Map();

window.registerTurboView = (name: string, render: (data: any, csrfToken: string) => ReactNode) => {
    turboViews.set(name, render);
};
...
export const FrameWrapper: React.FunctionComponent<FrameWrapperProps> = ({visible, mode, frame, style, onLoad, navigate, submitForm, openModal}) => {
    ...
    return (
        <iframe onLoad={onIframeLoad} style={newStyle} src={'/admin/turbo/frame/'} />
    );
}
