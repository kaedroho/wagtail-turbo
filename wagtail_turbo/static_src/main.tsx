...
const TurboWrapper = styled.div<WrapperProps>`
    height: 100vh;
...
const Turbo: React.FunctionComponent<TurboProps> = (props) => {
    const [collapsed, setCollapsed] = usePersistedState('wagtailturbo-collapsed', window.innerWidth < 800);
    ...
    return (
        <TurboWrapper collapsed={collapsed}>
...
export function initTurbo() {
    const turboElement = document.getElementById('wagtailturbo-root');
    const sidebarElement = document.getElementById('wagtailturbo-sidebar');

    if (turboElement instanceof HTMLElement && sidebarElement instanceof HTMLElement && sidebarElement.dataset.props && turboElement.dataset.initialResponse) {
        const navController = new NavigationController('browser', JSON.parse(turboElement.dataset.initialResponse), null);
        ...
        const renderTurbo = (_frame: Frame | null, firstRender?: boolean) => {
            ReactDOM.render(
                <Turbo {...props} navigationController={navController} />,
                turboElement,
                () => {
                    if (firstRender) {
        ...
        renderTurbo(null, true);
        navController.addNavigationListener(renderTurbo);
    }
}
