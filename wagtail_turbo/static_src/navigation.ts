import {turboGet, turboPost, TurboResponse} from './fetch';
...
export class NavigationController {
    ...
    constructor(mode: Mode, initialResponse: TurboResponse, parent: NavigationController | null) {
        this.mode = mode;
        this.title = '';
        this.parent = parent;
    ...
    _turboFetch = (fetcher: () => Promise<TurboResponse>, url: string, pushState: boolean): Promise<void> => {
        // Get a fetch ID
    ...
    _handleResponse = (response: TurboResponse, url: string, pushState: boolean = true) => {
    ...
    navigate = (url: string, pushState: boolean = true): Promise<void> => {
        return this._turboFetch(() => turboGet(url, this.mode), url, pushState);
    }

    submitForm = (url: string, data: FormData): Promise<void> => {
        return this._turboFetch(() => turboPost(url, data, this.mode), url, true);
    }

    escalate = (url: string, response: TurboResponse) => {
